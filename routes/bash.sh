...
if [[ $inputFile == '' || $targetFolder == '' ]]; then
...
malhadi_appleval() {
  ...
  header="${CYAN}`date +%H:%M:%S` ${NC}| from ${YELLOW}$inputFile ${NC}to ${YELLOW}$targetFolder${NC}"
  footer="[AppleValidator 2018]\n" 
  if [[ $val > 0 || $icl > 0 ]]; then
    printf "$header | $2/$3 | ${YELLOW}$1 ${NC}| ${GRN}LIVE ${NC}| $footer"
    echo "LIVE => $1" >> $4/live.txt
  else
    if [[ $inv > 0 || $bad > 0 ]]; then
      printf "$header | $2/$3 | ${YELLOW}$1 ${NC}| ${RED}DIE ${NC}| $footer"
      echo "DIE => $1" >> $4/die.txt
    else
      printf "$header | $2/$3 | ${YELLOW}$1 ${NC}| ${ORANGE}UNKNOWN ${NC}| $footer"
...
grep -Eiorha '([[:alnum:]_.-]+@[[:alnum:]_.-]+?\.[[:alpha:].]{2,6})' $inputFile | tr '[:upper:]' '[:lower:]' | sort | uniq > temp_list && mv temp_list $inputFile
...
echo " "
echo "Hotmail   : `egrep -c "@hotmail|@live|@outlook|@msn" $inputFile`"
echo "Yahoo : `egrep -c "@yahoo|@ymail|@btinternet|@bt|@rocketmail|@sky" $inputFile`"
echo "Gmail : `egrep -c "@gmail|@google|@googlemail" $inputFile`"
echo "Aol   : `egrep -c "@aol" $inputFile`"
echo "########################################"
...
for (( i = 0; i < "${#mailist[@]}"; i++ )); do
  ...
  if [[ $fold == 0 && $i > 0 ]]; then
    header="`date +%H:%M:%S`"
    duration=$SECONDS
    #echo -e "${CYAN}$header | Waiting $perSec second. $(($duration / 3600)) hours $(($duration / 60 % 60)) minutes and $(($duration % 60)) seconds elapsed."
    sleep $perSec
  fi
  vander=`expr $i % 8`
...
wait
#echo wc -l $targetFolder/*
echo -e "\n+==========+ LIVE : `wc -l $targetFolder/live.txt | cut -f1 -d' '`  |  DIE : `wc -l $targetFolder/die.txt | cut -f1 -d' '` +==========+\n"
